...
// ==========================================
// 2. ADMIN API (CRUD Kupon)
// ==========================================
couponRouter.get('/', async (c) => {
  const { results } = await c.env.DB.prepare('SELECT * FROM coupons ORDER BY created_at DESC').all();
  return c.json({ success: true, data: results });
});

couponRouter.post('/', async (c) => {
  const b = await c.req.json();
  const id = crypto.randomUUID();
  try {
    await c.env.DB.prepare(
      `INSERT INTO coupons (id, code, discount_type, discount_value, min_purchase, max_discount, valid_until, usage_limit) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      id, b.code.toUpperCase(), b.discount_type, parseInt(b.discount_value) || 0, parseInt(b.min_purchase) || 0,
      parseInt(b.max_discount) || 0, b.valid_until || null, parseInt(b.usage_limit) || 0
    ).run();
    return c.json({ success: true, message: 'Kupon berhasil dibuat.' });
  } catch(e) {
    return c.json({ success: false, message: 'Kode kupon mungkin sudah ada.' }, 400);
  }
});

couponRouter.delete('/:id', async (c) => {
  await c.env.DB.prepare('DELETE FROM coupons WHERE id = ?').bind(c.req.param('id')).run();
  return c.json({ success: true });
});

couponRouter.put('/:id/status', async (c) => {
  const { is_active } = await c.req.json();
  await c.env.DB.prepare('UPDATE coupons SET is_active = ? WHERE id = ?').bind(is_active, c.req.param('id')).run();
  return c.json({ success: true });
});
