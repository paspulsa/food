...
export default createRoute(async (c) => {
  // 1. Ambil data pengaturan, jika kosong berikan nilai default (Fallback)
  let settings = await c.env.DB.prepare('SELECT * FROM delivery_settings WHERE id = ?').bind('default-settings').first<any>();
  
  // Auto-Insert jika tabel delivery_settings masih kosong
  if (!settings) {
    const defaultData = { id: 'default-settings', free_range_max: 2, mid_range_max: 5, mid_range_price: 5000, max_range_price: 10000, max_radius_limit: 10 };
    ...
    ).bind(defaultData.id, defaultData.free_range_max, defaultData.mid_range_max, defaultData.mid_range_price, defaultData.max_range_price, defaultData.max_radius_limit).run();
    settings = defaultData;
  }

  // 2. Handler untuk menyimpan data (POST Request)
    ...
    <div class="space-y-6 max-w-4xl mx-auto">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-darkpanel p-6 rounded-2xl border border-gray-100 dark:border-darkborder shadow-sm">
        <div>
          <h2 class="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
            <svg class="w-7 h-7 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
      ...
      <form onsubmit="event.preventDefault(); saveDeliverySettings();" class="space-y-6">
        ...
        <div class="bg-white dark:bg-darkpanel rounded-2xl border border-gray-100 dark:border-darkborder shadow-sm overflow-hidden group hover:border-green-300 dark:hover:border-green-500/50 transition-colors">
          ...
          <div class="p-6">
            ...
            <div class="relative max-w-md">
              <input type="number" step="any" id="free_range" value={settings.free_range_max} required class="w-full pl-4 pr-12 py-3 bg-gray-50 dark:bg-darkbg border border-gray-200 dark:border-gray-700 rounded-xl text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all font-mono" />
              <span class="absolute right-4 top-1/2 -translate-y-1/2 font-bold text-gray-400">KM</span>
            </div>
            <p class="text-[11px] text-gray-400 mt-2 italic">*Contoh: Jika diisi 2, maka jarak 0 hingga 2 KM akan gratis ongkir.</p>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-darkpanel rounded-2xl border border-gray-100 dark:border-darkborder shadow-sm overflow-hidden group hover:border-yellow-300 dark:hover:border-yellow-500/50 transition-colors">
          ...
          <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <div class="relative">
                <input type="number" step="any" id="mid_range" value={settings.mid_range_max} required class="w-full pl-4 pr-12 py-3 bg-gray-50 dark:bg-darkbg border border-gray-200 dark:border-gray-700 rounded-xl text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all font-mono" />
                <span class="absolute right-4 top-1/2 -translate-y-1/2 font-bold text-gray-400">KM</span>
              </div>
            </div>
            <div>
        ...
        <div class="bg-white dark:bg-darkpanel rounded-2xl border border-gray-100 dark:border-darkborder shadow-sm overflow-hidden group hover:border-red-300 dark:hover:border-red-500/50 transition-colors">
          <div class="p-5 bg-red-50/50 dark:bg-red-500/10 border-b border-gray-100 dark:border-darkborder flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400">Tarif tertinggi dan sistem penolakan otomatis jika terlalu jauh.</p>
            </div>
          </div>
          <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-gray-400">Rp</span>
                <input type="number" id="max_price" value={settings.max_range_price} required class="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-darkbg border border-gray-200 dark:border-gray-700 rounded-xl text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all font-mono" />
              </div>
        ...
        <div class="flex justify-end pt-4">
          <button id="btnSubmit" type="submit" class="w-full sm:w-auto bg-primary hover:bg-primary/90 text-white font-bold py-3.5 px-8 rounded-xl shadow-lg shadow-primary/20 transition-all flex items-center justify-center gap-2 active:scale-95">
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        ...
        function showToast(message, isError = false) {
          ...
          toast.innerHTML = isError 
            ? '<svg class="w-5 h-5 text-red-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> ' + message
  ...
  , { title: 'Pengaturan Jarak & Ongkir - Admin SPOS' })
})
