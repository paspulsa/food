...
export default createRoute(async (c) => {
  ...
  // ==========================================
  // 2. TARIK DATA KATALOG DARI DATABASE
  // ==========================================
  ...
  return c.render(
    <div class="bg-gray-100 min-h-screen font-sans">
      ...
      <div class="max-w-md mx-auto bg-gray-50 min-h-screen relative shadow-2xl pb-24 overflow-x-hidden">
        ...
        {/* KONTEN UTAMA (Slider, Grid Kategori, Flash Sale, Rekomendasi) */}
        <div class="w-full">
          {/* Banner Promo */}
          <div class="px-4 mt-4">
        ...
        <div class="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full max-w-md bg-white border-t border-gray-200 shadow-[0_-4px_10px_-1px_rgba(0,0,0,0.08)] z-50">
      ...
      {/* SCRIPT INTERAKTIF (LOKASI GPS & KERANJANG) */}
      <script dangerouslySetInnerHTML={{ __html: `
        ...
        function promptManualLocation() {
          const newAddress = prompt("Masukkan alamat pengiriman Anda secara lengkap:");
          if (newAddress && newAddress.trim() !== "") {
            // (Opsional) Di tahap produksi, jika DB_ADDRESS ada (user login), Anda bisa memanggil API PUT untuk update kolom address di tabel users
            
            // Simpan ke sesi lokal
            localStorage.setItem('user_saved_address', newAddress);
            document.getElementById('user-location').innerHTML = \`<span class="truncate">\${newAddress}</span> <svg class="w-4 h-4 ml-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>\`;
          }
        }
        ...
        function addToCart(id, name, price) {
          ...
          toast.className = 'fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-gray-900/90 backdrop-blur-sm border border-gray-800 text-white text-[11px] font-bold px-5 py-3 rounded-full shadow-2xl z-50 flex items-center gap-2 transition-all duration-300 opacity-0 translate-y-4';
